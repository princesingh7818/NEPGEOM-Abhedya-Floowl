...
import importlib
prop_algo = importlib.import_module('prop-algo')
...
def run(bounds, token, output_dir=None, zoom=None, expand_factor=2.0,
    ...
    if algorithm == 'exp-hand':
        ...
        osm_json = prop_algo.fetch_osm_rivers(expanded_bounds)
        osm_mask, osm_weight = prop_algo.rasterize_river_geojson(osm_json, transform, dem.shape)
        if not osm_mask.any():
            osm_mask = river_mask
            osm_weight = np.where(river_mask, 1.0, 0.0)
        flood_score = prop_algo.compute_test_algo(
